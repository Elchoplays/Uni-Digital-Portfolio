...
const heroBackgrounds = [
  {
    image: testImage,
    alt: 'Aerial tropical coastline',
    position: 'center center',
  },
  {
    image: 'https://upload.wikimedia.org/wikipedia/commons/2/22/Antelope_Canyon.jpg',
    alt: 'Smooth layered sandstone canyon walls of Antelope Canyon',
    position: 'center center',
  },
  {
    image: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=2070&auto=format&fit=crop',
    alt: 'Striking mountain landscape showing distinct rock stratigraphy',
    position: 'center center',
  },
  {
    image: 'https://upload.wikimedia.org/wikipedia/commons/f/f5/Grand_prismatic_spring.jpg',
    alt: 'Vibrant geothermal features of the Grand Prismatic Spring',
    position: 'center center',
  },
  {
    image: 'https://upload.wikimedia.org/wikipedia/commons/5/58/Badlands_at_Sunset.jpg',
    alt: 'Striking layered geological formations of the Badlands at sunset',
    position: 'center center',
  }
];
...
const Hero: React.FC = () => {
  ...
  const activeBackground = heroBackgrounds[activeBackgroundIndex];
  ...
  const stepBackground = (direction: 1 | -1) => {
    setActiveBackgroundIndex((currentIndex) => {
      ...
      if (nextIndex < 0) {
        return heroBackgrounds.length - 1;
      }

      if (nextIndex >= heroBackgrounds.length) {
        return 0;
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveBackgroundIndex((currentIndex) => {
        ...
        return nextIndex >= heroBackgrounds.length ? 0 : nextIndex;
      });
    }, 30000);
  ...
  return (
    <section id="home" className="relative min-h-screen flex flex-col justify-center items-center text-white text-center overflow-hidden px-4">
      ...
      <div className="absolute top-0 left-0 w-full h-full z-0">
        <AnimatePresence mode="wait" initial={false}>
          <motion.img
            ...
            initial={shouldReduceMotion ? { opacity: 1 } : { opacity: 0, scale: 1.1 }}
            animate={{ opacity: 1, scale: 1.05 }}
            exit={shouldReduceMotion ? { opacity: 1 } : { opacity: 0, scale: 1.02 }}
            transition={shouldReduceMotion ? { duration: 0.01 } : { duration: 0.75, ease: 'easeOut' }}
          />
        </AnimatePresence>
        {/* Overlay */}
        ...
        />
        <div className="absolute bottom-6 right-4 z-10 flex items-center gap-2 sm:bottom-8 sm:right-8">
          <motion.button
            type="button"
            onClick={() => stepBackground(-1)}
            className="grid h-11 w-11 place-items-center rounded-full border border-white/30 bg-black/25 text-white backdrop-blur-sm transition-colors duration-300 hover:bg-black/40"
            aria-label="Show previous hero background"
            whileHover={shouldReduceMotion ? { scale: 1.01 } : { y: -2, scale: 1.03 }}
            whileTap={shouldReduceMotion ? { scale: 0.98 } : { scale: 0.95 }}
          >
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
            </svg>
          </motion.button>
          <div className="flex items-center gap-2 rounded-full border border-white/20 bg-black/20 px-3 py-2 backdrop-blur-sm">
            {heroBackgrounds.map((background, index) => (
              <button
                key={background.image}
                type="button"
                onClick={() => goToBackground(index)}
                className={`h-2.5 rounded-full transition-all duration-300 ${index === activeBackgroundIndex
                  ? 'w-8 bg-white'
                  : 'w-2.5 bg-white/45 hover:bg-white/70'
                  }`}
                aria-label={`Show hero background ${index + 1}`}
                aria-pressed={index === activeBackgroundIndex}
              />
            ))}
          </div>
          <motion.button
            ...
            onClick={() => stepBackground(1)}
            className="grid h-11 w-11 place-items-center rounded-full border border-white/30 bg-black/25 text-white backdrop-blur-sm transition-colors duration-300 hover:bg-black/40"
            aria-label="Show next hero background"
            whileHover={shouldReduceMotion ? { scale: 1.01 } : { y: -2, scale: 1.03 }}
            whileTap={shouldReduceMotion ? { scale: 0.98 } : { scale: 0.95 }}
          >
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-5xl rounded-3xl border border-white/20 bg-white/5 px-6 py-14 backdrop-blur-[2px] sm:px-10 sm:py-16 dark:border-white/10 dark:bg-slate-950/15"
      >
        ...
        <motion.p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-gray-200/95 sm:text-lg dark:text-slate-200" {...reveal(0.28, 14)}>
        ...
        >
          <div className="flex justify-center gap-4 sm:gap-6">
            <motion.a href="https://github.com/Elchoplays" target="_blank" rel="noopener noreferrer" className="grid h-12 w-12 place-items-center rounded-full border border-white/30 bg-white/10 text-white transition-colors duration-300 hover:bg-white/25 dark:border-white/20 dark:bg-slate-900/35 dark:hover:bg-slate-800/55" aria-label="GitHub Profile" whileHover={{ y: -3, scale: 1.06 }} whileTap={{ scale: 0.96 }}>
            ...
            <motion.a href="https://www.linkedin.com/in/eleanor-seaton/" target="_blank" rel="noopener noreferrer" className="grid h-12 w-12 place-items-center rounded-full border border-white/30 bg-white/10 text-white transition-colors duration-300 hover:bg-white/25 dark:border-white/20 dark:bg-slate-900/35 dark:hover:bg-slate-800/55" aria-label="LinkedIn Profile" whileHover={{ y: -3, scale: 1.06 }} whileTap={{ scale: 0.96 }}>
            ...
            <motion.a href="https://arup-my.sharepoint.com/personal/eleanor_seaton_arup_com/documents/documents/cv/cv%20final%20verision%202025.docx" target="_blank" rel="noopener noreferrer" className="grid h-12 w-12 place-items-center rounded-full border border-white/30 bg-white/10 text-white transition-colors duration-300 hover:bg-white/25 dark:border-white/20 dark:bg-slate-900/35 dark:hover:bg-slate-800/55" aria-label="View CV" whileHover={{ y: -3, scale: 1.06 }} whileTap={{ scale: 0.96 }}>
